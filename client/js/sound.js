...
function drawTrack(decodedBuffer, trackNumber) {
    ...
    console.log("drawTrack : let's draw sample waveform for track No" + trackNumber + " named " +
        ...
        /*Eliminé los titulos en la repoducción de las ondas */

    var trackName = currentSong.tracks[trackNumber].name;
    //trackName = trackName.slice(trackName.lastIndexOf("/")+1, trackName.length-4);
    /*Color onda reproducción */
    waveformDrawer.init(decodedBuffer, View.masterCanvas, 'rgb(78,75,72)');
    var x = 0;
    ...
    /*cambia la medida del ancho de la onda de la canción*/
    
    View.masterCanvasContext.strokeStyle = "rgb(121,192,0)";
    /*Color borde reproducción */
    View.masterCanvasContext.strokeRect(x, y, window.View.masterCanvas.width, SAMPLE_HEIGHT);
    View.masterCanvasContext.fonteight = 740;
    View.masterCanvasContext.lineWidth = 2;
...
function finishedLoading(bufferList) {
    ...
    // Set each track volume slider to max, OJOOOOOO Habla de los slider personales
    ...
    xhr.onload = function (e) {
        ...
        song.instruments.forEach(function (instrument, trackNumber) {
            ...
            span.innerHTML = '<td class="trackBox" style="height : ' + SAMPLE_HEIGHT + 'px">' +
                ...
                "<button class='solo' id='solo" + trackNumber + "' onclick='soloNosoloTrack(" + trackNumber + ");'><img src='../img/no_solo.png' /></button></div>" +
                "<button class='mute' id='mute" + trackNumber + "' onclick='muteUnmuteTrack(" + trackNumber + ");'><img src='../img/earphones.png' /></button></div> " +
...
function animateTime() {
    ...
    if (currentSong !== undefined) {
        ...
        if (!currentSong.paused) {
            ...
            var totalTime;
/*  Este me determina el estilo del contador*/
            View.frontCanvasContext.fillStyle = 'yellow'; 
            View.frontCanvasContext.font = '22pt Century';
            ...
            if (currentSong.decodedAudioBuffers[0] !== undefined) {
                ...
                // Draw time bar
                View.frontCanvasContext.strokeStyle = "black";
                View.frontCanvasContext.lineWidth = 3;
                View.frontCanvasContext.beginPath();
                View.frontCanvasContext.moveTo(currentXTimeline, 38);
...
/*OJOOO Aquí puedo configurar la flechas del incio, letras, colores y demás*/
function showWelcomeMessage() {
    View.frontCanvasContext.save();
    View.frontCanvasContext.font = '20pt Times New Roman';
    View.frontCanvasContext.fillStyle = 'black';
/*Letra Bienvenida*/
    View.frontCanvasContext.fillText('Welcome to Software de ejecución instrumental', 200, 120); 
...
function drawFrequencies() {
    ...
    View.waveCanvasContext.strokeStyle = "yellow";
    /*cambia el borde de la onda de arriba */
...
function drawSampleImage(imageURL, trackNumber, trackName) {
    ...
    image.onload = function () {
        ...
        View.masterCanvasContext.fillStyle = '#FF0000';
...
function setMasterVolume(val) {
    if (currentSong !== undefined) {
        ...
        document.querySelector("#bsound").innerHTML = "<img src='../img/earphones.png' /></button></div>";
        var fraction;

        // set its volume to the current value of the master volume knob, OJOOOOO ESTO PUEDE SERVIRRRRR
        if (val=== undefined) {
...
function soloNosoloTrack(trackNumber) {
    ...
    // Let's change the icon
    m.innerHTML = "<img src='../img/earphones.png' />";
...
function muteUnmuteTrack(trackNumber) {
    ...
    if (!currentTrack.muted) {
        ...
        m.innerHTML = "<img src='../img/noearphones.png' />";
    } else {
...
function masterMuteUnmute(btn) {
    ...
    if (currentSong.muted) {
        btn.innerHTML = "<img src='../img/noearphones.png' />";
    } else {
        btn.innerHTML = "<img src='../img/earphones.png' />";
    }
}
